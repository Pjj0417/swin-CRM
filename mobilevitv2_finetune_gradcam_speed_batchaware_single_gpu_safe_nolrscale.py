# --------------------------------------------------------
# MobileViTv2 backbone: local pretrained ImageNet-1K weights
...
from models.mobilevitv2_model import build_mobilevitv2
...
MOBILEVITV2_MODEL_NAME = "mobilevitv2_100"
MOBILEVITV2_PRETRAINED_PATH = None
...
GRADCAM_BEST_ONLY = False



def parse_option():
    parser = argparse.ArgumentParser(
        "MobileViTv2 training, evaluation and Grad-CAM++ script",
        add_help=True,
    )
    ...
    parser.add_argument(
        "--freeze-backbone",
        action="store_true",
        help="Freeze MobileViTv2 backbone and train only the classifier.",
    )
    parser.add_argument(
        "--optimizer",
        type=str,
...
def find_mobilevitv2_gradcam_target_layers(model_without_ddp, logger=None):
    """
    Select the last Conv2d in the feature backbone for Grad-CAM++.
    The selected layer returns a BCHW spatial activation map.
    """
    if not hasattr(model_without_ddp, "backbone"):
        raise AttributeError("Model does not contain a MobileViTv2 backbone.")

    target_layer = None
    target_name = None

    for name, module in model_without_ddp.backbone.named_modules():
    ...
    if target_layer is None:
        raise RuntimeError(
            "Could not find a Conv2d layer for MobileViTv2 Grad-CAM++."
        )

    if logger is not None:
        logger.info(
            "Selected MobileViTv2 Grad-CAM++ target layer: "
...
def save_best_weights(
    ...
    checkpoint = {
        ...
        "architecture": MOBILEVITV2_MODEL_NAME,
        "initialization": "MobileViTv2_ImageNet1K_pretrained",
    }
...
def save_gradcam_images(
    ...
    try:
        target_layers = find_mobilevitv2_gradcam_target_layers(
            model_without_ddp,
            logger,
        )
...
def main(config, skip_gradcam=False):
    ...
    logger.info(f"Creating MobileViTv2 model: {MOBILEVITV2_MODEL_NAME}")

    model = build_mobilevitv2(
        num_classes=int(config.MODEL.NUM_CLASSES),
        model_name=MOBILEVITV2_MODEL_NAME,
        drop_rate=float(config.MODEL.DROP_RATE),
        drop_path_rate=float(config.MODEL.DROP_PATH_RATE),
        freeze_backbone=FREEZE_BACKBONE,
        pretrained_path=MOBILEVITV2_PRETRAINED_PATH,
    )
    model.cuda()
...
def throughput(data_loader, model, logger):
    """
    Measure batched MobileViTv2 inference throughput.
...
if __name__ == "__main__":
    ...
    MOBILEVITV2_MODEL_NAME = args.mobilevitv2_model
    MOBILEVITV2_PRETRAINED_PATH = args.pretrained
    ...
    # For a single GPU, use Gloo only to preserve the existing distributed
    # sampler/rank API without touching NCCL/UCX. Multi-GPU keeps NCCL.
    distributed_backend = (
        "nccl"
        if world_size > 1
        else "gloo"
    ...
    # MobileViTv2 fine-tuning:
    # use the learning rates from YAML directly.
    #
    # The original SimMIM code scales LR by global_batch_size / 512.
    # With batch_size=32 that turns 3e-5 into 1.875e-6, which is far too
    # conservative for a newly initialized binary classification head.
    # Therefore no additional /512 scaling is applied here.
    print(
        "MobileViTv2 fine-tune LR (no SimMIM /512 scaling): "
...
#   mobilevitv2_finetune_gradcam_speed_batchaware_single_gpu_safe_nolrscale.py \
#   --cfg ./configs/mobilevitv2/mobilevitv2_100_finetune_img224.yaml \
#   --data-path ./data/fatiguev2_105270 \
#   --batch-size 32 \
#   --amp-opt-level O0 \
#   --mobilevitv2-model mobilevitv2_100 \
#   --pretrained /root/shared-nvme/uploads/mobilevitv2_100_cvnets_in1k.bin \
#   --skip-gradcam \
#   --tag mobilevitv2_100_100ep \
#   --opts \
#   TRAIN.EPOCHS 100 \
