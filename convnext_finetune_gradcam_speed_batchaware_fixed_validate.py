# --------------------------------------------------------
# ConvNeXt backbone: local pretrained ImageNet weights
...
def parse_option():
    parser = argparse.ArgumentParser(
        "ConvNeXt training, evaluation and Grad-CAM++ script",
        add_help=True,
    )
    ...
    parser.add_argument(
        "--freeze-backbone",
        action="store_true",
        help="Freeze ConvNeXt backbone and train only the classifier.",
    )
    parser.add_argument(
        "--optimizer",
        type=str,
...
def find_convnext_gradcam_target_layers(model_without_ddp, logger=None):
    if not hasattr(model_without_ddp, "backbone"):
        raise AttributeError("Model does not contain a ConvNeXt backbone.")

    backbone = model_without_ddp.backbone

    if not hasattr(backbone, "stages") or len(backbone.stages) == 0:
        raise AttributeError(
            "ConvNeXt backbone does not expose timm-style stages."
        )
    ...
    if not hasattr(last_stage, "blocks") or len(last_stage.blocks) == 0:
        raise AttributeError(
            "ConvNeXt final stage does not contain blocks."
        )

    last_block = last_stage.blocks[-1]

    # conv_dw emits a normal BCHW feature map, which Grad-CAM++ can consume
    # directly. No ViT/token reshape_transform is needed for ConvNeXt.
    if hasattr(last_block, "conv_dw"):
        target_layer = last_block.conv_dw
        target_name = "backbone.stages[-1].blocks[-1].conv_dw"
    else:
        target_layer = last_block
        target_name = "backbone.stages[-1].blocks[-1]"

    if logger is not None:
        logger.info(
            "Selected ConvNeXt Grad-CAM++ target layer: "
            f"{target_name}"
        )
...
def save_best_weights(
    ...
    checkpoint = {
        ...
        "architecture": CONVNEXT_MODEL_NAME,
        "initialization": "ConvNeXt_local_pretrained",
    }
...
def save_gradcam_images(
    ...
    try:
        target_layers = find_convnext_gradcam_target_layers(
            model_without_ddp,
            logger,
        )
...
def main(config, skip_gradcam=False):
    ...
    logger.info(f"Creating ConvNeXt model: {CONVNEXT_MODEL_NAME}")
    ...
    model = build_convnext(
        ...
        pretrained_path=CONVNEXT_PRETRAINED_PATH,
    )
    model.cuda()
    ...
    classifier_trainable = sum(
        parameter.numel()
        for parameter in model_without_ddp.classifier.parameters()
        if parameter.requires_grad
    )
    backbone_trainable = sum(
        parameter.numel()
        for parameter in model_without_ddp.backbone.parameters()
        if parameter.requires_grad
    )
    logger.info(
        "Trainable parameter split: "
...
def throughput(data_loader, model, logger):
    """
    Measure batched ConvNeXt inference throughput.
...
if __name__ == "__main__":
    ...
    CONVNEXT_MODEL_NAME = args.convnext_model
    CONVNEXT_PRETRAINED_PATH = args.pretrained
    ...
    # use learning rates from YAML directly.
    #
    # The original SimMIM code scales LR by global_batch_size / 512.
    # With batch_size=32 this makes fine-tuning LR unnecessarily small,
    # especially for the newly initialized binary classification head.
    print(
        "ConvNeXt fine-tune LR (no SimMIM /512 scaling): "
...
# convnext_finetune_gradcam_speed_batchaware_fixed_validate.py \
# --cfg ./configs/convnext/convnext_tiny_finetune_img224.yaml \
...
# --skip-gradcam \
# --tag uta_rldd_convnext_tiny_10ep_speed \
# --opts \
# TRAIN.EPOCHS 10 \
