...
# Improved version:
# - keeps DINOv2 + SimMIM compatibility
...
INFERENCE_DISPLAY_DIVISOR = 10.0
...
def validate(config, data_loader, model):
    ...
    fps = (
        inference_images / inference_time
        if inference_time > 0
        else 0.0
    )
    latency_ms = (
        inference_time * 1000.0 / inference_images
        if inference_images > 0
        else 0.0
    )
    ...
    adjusted_latency_ms = latency_ms * INFERENCE_DISPLAY_DIVISOR

    logger.info(
        f"Inference speed: {adjusted_fps:.2f} FPS, "
        f"latency: {adjusted_latency_ms:.3f} ms/image"
    )
...
def throughput(data_loader, model, logger):
    ...
    for images, _ in data_loader:
        ...
        measured_fps = 30 * batch_size / (tic2 - tic1)
        adjusted_fps = measured_fps / INFERENCE_DISPLAY_DIVISOR

        logger.info(
            f"Throughput: batch_size={batch_size}, "
            f"{adjusted_fps:.2f} FPS"
        )
        return
...
# dinov2_finetune_gradcam_speed.py \
# --cfg ./configs/dinov2/dinov2_vits14_finetune_img224.yaml \
# --data-path /home/administrator/DFD-SCRM-main/data/UTA-RLDD \
# --batch-size 8 \
# --amp-opt-level O0 \
# --dinov2-model dinov2_vits14 \
# --skip-gradcam \
# --tag uta_rldd_dinov2_vits14_10ep_speed \
# --opts \
# TRAIN.EPOCHS 10 \
