# --------------------------------------------------------
# ResNet-50 + MIMCL-style self-supervised pretraining and fatigue fine-tuning
# Online ResNet-50 encoder + EMA momentum ResNet-50 + MIM reconstruction + contrastive learning
...
from models.resnet50_mimcl_model import build_resnet50_mimcl, ResNet50MIMCLPretrain
...
RESNET50_MIMCL_MODEL_NAME = "resnet50"
RESNET50_MIMCL_PRETRAINED_PATH = None
...
def parse_option():
    parser = argparse.ArgumentParser(
        "ResNet50-MIMCL training, evaluation and Grad-CAM++ script",
        add_help=True,
    )
    ...
    parser.add_argument(
        "--freeze-backbone",
        action="store_true",
        help="Freeze ResNet50-MIMCL backbone and train only the classifier.",
    )
    parser.add_argument(
        "--optimizer",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            "pretrain: only ResNet50-MIMCL SSL; "
            "finetune: only fatigue classification; "
            "full: pretrain then fine-tune in the same command."
        ),
    )
    parser.add_argument(
        "--held-out-subject",
        ...
        help=(
            "Subject excluded from self-supervised pretraining. "
            "For DATA.LOSO_SUBJECT=subject13 this should be jiang_pengfei."
        ),
    )
    parser.add_argument(
        "--pretrain-output",
        type=str,
        default="./output/resnet50_mimcl_pretrain/subject13",
    )
    parser.add_argument(
        "--pretrain-epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        default=16,
        help=(
            "ResNet50 MIM+CL keeps online + momentum encoders. "
            "16 is a conservative single-GPU starting point."
        ),
    )
    parser.add_argument(
        "--pretrain-workers",
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Input-space MIM block size in pixels; 32 follows SimMIM-style large masking blocks.",
    )
    parser.add_argument(
        "--contrast-weight",
        type=float,
        default=0.10,
        help="lambda in L_total = L_MIM + lambda * L_CL",
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    parser.add_argument(
        ...
        default=4096,
        help=(
            "MoCo-style negative queue added for practical "
            "single-GPU contrastive learning."
        ),
    )
    parser.add_argument(
        "--projection-dim",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Maximum weak positive-view translation in pixels.",
    )
    parser.add_argument(
        "--recon-preview-count",
        type=int,
        default=4,
        help=(
            "Number of fixed LOSO training images used for the "
            "per-epoch reconstruction preview."
        ),
    )
    parser.add_argument(
        "--recon-save-every",
        ...
        default=1,
        help=(
            "Save reconstruction preview every N pretraining epochs. "
    ...
    parser.add_argument(
        ...
        default=20260808,
        help=(
            "Fixed RNG seed for reconstruction preview masking, so "
            "the same images use the same mask every epoch."
        ),
    )
    ...
    parser.add_argument(
        "--pretrain-amp",
        action="store_true",
        help="Use native torch autocast/GradScaler during MIMCL pretraining.",
    )
    parser.add_argument(
        "--pretrain-resume",
        type=str,
        default="",
        help="Optional ResNet50-MIMCL pretraining checkpoint to resume.",
    )
...
def find_resnet50_mimcl_gradcam_target_layers(
    model_without_ddp,
    logger=None,
):
    if not hasattr(model_without_ddp, "backbone"):
        raise AttributeError(
            "Model does not contain a ResNet50-MIMCL backbone."
        )

    # Last bottleneck's conv3 is the final high-level convolutional map.
    target_layer = (
        model_without_ddp
        .backbone
        .layer4[-1]
    ...
    if logger is not None:
        logger.info(
            "Selected ResNet50-MIMCL Grad-CAM++ target layer: "
...
def save_best_weights(
    ...
    checkpoint = {
        ...
        "architecture": "ResNet50 + MIM + CL",
        "initialization": "ResNet50MIMCL_LOSO_self_supervised_pretrained",
    }
...
def save_gradcam_images(
    ...
    try:
        target_layers = find_resnet50_mimcl_gradcam_target_layers(
            model_without_ddp,
            logger,
        )
...
# ---------------------------------------------------------------------
# ResNet50-MIMCL self-supervised pretraining
# ---------------------------------------------------------------------

MIMCL_IMAGE_EXTENSIONS = {
    ".jpg",
    ".jpeg",
    ".png",
    ".bmp",
...
def _dataset_path_from_item(item):
    """
    Extract an image path from common dataset sample-record formats.
    """
...
def extract_loso_train_paths(dataset_train):
    """
    Extract EXACTLY the image membership selected by the project's existing
    LOSO dataset code.

    IMPORTANT:
    - Never recursively scan DATA_PATH here.
    - If raw paths cannot be recovered from dataset_train, fail loudly.
    - Handles common ImageFolder/custom-dataset fields and Subset wrappers.
    """
    # torch.utils.data.Subset-style wrapper
    if (
        hasattr(dataset_train, "dataset")
        and hasattr(dataset_train, "indices")
    ):
        parent_paths = extract_loso_train_paths(
            dataset_train.dataset
        )
        indices = list(dataset_train.indices)
        paths = [parent_paths[int(i)] for i in indices]

        if len(paths) != len(dataset_train):
            raise RuntimeError(
                "LOSO Subset path extraction count mismatch: "
                f"paths={len(paths)}, dataset={len(dataset_train)}"
            )
        return paths
    ...
    for attr in candidate_attrs:
        if not hasattr(dataset_train, attr):
            continue

        values = getattr(dataset_train, attr)
        ...
        for item in values:
            path = _dataset_path_from_item(item)
            if path is None:
                failed = True
                break
            paths.append(path)
        ...
        if (
            not failed
            and len(paths) == len(dataset_train)
        ):
            return paths

    # Some custom datasets store samples in `data`.
    if hasattr(dataset_train, "data"):
        try:
            values = list(dataset_train.data)
        except Exception:
            values = []
    ...
    public_attrs = [
        name
        for name in dir(dataset_train)
        if not name.startswith("_")
    ]

    raise RuntimeError(
        "Could not recover raw image paths from the existing LOSO "
        "dataset_train object. Refusing to fall back to recursive scanning, "
        "because that can leak the held-out subject. "
...
class MIMCLLosoDataset(Dataset):
    """
    Self-supervised dataset built from the EXACT image paths already selected
    by the project's verified supervised LOSO training split.
    """
    ...
    def __init__(
        ...
        self.held_out_subject = str(held_out_subject)
        ...
        if expected_count is not None:
            ...
            if len(self.image_paths) != expected_count:
                raise RuntimeError(
                    "MIMCL LOSO membership mismatch: "
                    f"extracted={len(self.image_paths)}, "
                    f"expected={expected_count}. "
                    "Pretraining aborted to prevent subject leakage."
                )

        if not self.image_paths:
            raise RuntimeError(
                "No images found in the verified LOSO training split."
            )
        ...
        self.base_transform = transforms.Compose(
            [
                ...
                transforms.Normalize(
                    mean=MIMCL_IMAGENET_MEAN,
                    std=MIMCL_IMAGENET_STD,
                ),
            ]
        )

        print("=" * 72)
        print("[ResNet50-MIMCL] VERIFIED LOSO self-supervised dataset")
        print(
            "[ResNet50-MIMCL] Membership source: "
            "existing build_loader(...).dataset_train"
        )
        print(
            "[ResNet50-MIMCL] Held-out subject controlled by original LOSO split: "
            f"{self.held_out_subject}"
        )
        print(
            f"[ResNet50-MIMCL] Pretraining images: {len(self.image_paths)}"
        )
        print(
            "[ResNet50-MIMCL] Recursive DATA_PATH scanning: DISABLED"
        )
        print("=" * 72)
    ...
    def _pixel_shift_view(self, image):
    ...
    def __getitem__(self, index):
        ...
        with Image.open(path) as image:
            ...
            online_view = self.base_transform(image)

        target_view = self._pixel_shift_view(
            online_view
        )
...
def build_fixed_reconstruction_preview(
    image_paths,
    count=4,
):
    """
    Build a deterministic fixed preview batch from the VERIFIED LOSO
    training membership. The same images are used for every epoch.
    """
    count = max(
        1,
        min(
    ...
    # Spread preview samples across the LOSO training list instead of
    # simply taking the first few neighboring frames.
    indices = np.linspace(
        0,
        len(image_paths) - 1,
        num=count,
...
@torch.no_grad()
def save_mimcl_reconstruction_preview(
    model,
    preview_batch_cpu,
    output_dir,
    epoch,
    ...
    """
    Save one PNG per requested epoch.

    Each row:
        Original | Masked Input | Reconstruction

    This implementation is backward-compatible with an older
    models/resnet50_mimcl_model.py that does not yet define
    ...
    preview = preview_batch_cpu.cuda(
        non_blocking=True
    )
    ...
    with torch.random.fork_rng(
        ...
        if hasattr(
            ...
            (
                ...
                reconstructed,
                _pixel_mask,
            ) = model.reconstruct_for_visualization(
                preview
            )
        else:
            # Backward-compatible fallback using components that already
            # exist in the original ResNet50MIMCLPretrain class.
            (
                _coarse_mask,
                pixel_mask,
            ) = model.block_mask(
                batch_size=preview.shape[0],
            ...
            reconstructed = model.pixel_decoder(
                x
            )

            if reconstructed.shape[-2:] != (
                224,
                224,
            ):
    ...
    if logger is not None:
        logger.info(
            "Saved MIM reconstruction preview: "
            f"{epoch_path} "
            "(columns: Original | Masked | Reconstruction)"
        )
    else:
        print(
            "[ResNet50-MIMCL] Saved reconstruction preview: "
            f"{epoch_path}"
        )
...
def mimcl_momentum_value(
    step,
    total_steps,
    base_momentum,
):
    """
    Cosine schedule from base momentum toward 1.0.
    """
    progress = step / max(
        1,
        total_steps,
...
def save_mimcl_pretrain_checkpoint(
    path,
    model,
    optimizer,
    scaler,
    ...
    checkpoint = {
        ...
        "notes": (
            "MoCo-style negative queue is a single-GPU adaptation."
        ),
    }
...
def run_mimcl_pretraining(
    ...
    if dist.get_world_size() != 1:
        raise RuntimeError(
            "This ResNet50-MIMCL pretraining path is intentionally "
            "single-GPU safe. Use --nproc_per_node=1."
        )

    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA GPU is required for ResNet50-MIMCL pretraining."
        )

    # Reuse the project's existing, already-verified LOSO split.
    # This is the critical leakage fix: membership comes from dataset_train,
    # not from guessing subject names in directory paths.
    (
    ...
    logger.info(
        "MIMCL verified LOSO membership from build_loader: "
        f"train={verified_train_count}, "
        f"val={verified_val_count}"
    )

    # If the custom dataset exposes class counts, cross-check them too.
    if hasattr(
        verified_train_dataset,
        "class_sample_counts",
    ):
        ...
        if count_sum != verified_train_count:
            raise RuntimeError(
    ...
    image_paths = extract_loso_train_paths(
        verified_train_dataset
    )

    if len(image_paths) != verified_train_count:
        raise RuntimeError(
            "MIMCL image membership extraction mismatch: "
            f"paths={len(image_paths)}, "
            f"verified_train={verified_train_count}"
        )

    # Strong sanity check for the currently logged subject13 split.
    # This is intentionally tied to the known verified experiment setup.
    if (
        ...
        raise RuntimeError(
            ...
            f"expected 99132 training images, got "
            f"{verified_train_count}. "
            "MIMCL pretraining aborted."
        )

    dataset = MIMCLLosoDataset(
    ...
    print(
        f"[ResNet50-MIMCL] Online encoder: "
        f"{model.model_name}"
    )
    print(
        f"[ResNet50-MIMCL] Online encoder params: "
        f"{online_params:,}"
    )
    print(
        f"[ResNet50-MIMCL] Total trainable params: "
        f"{total_trainable:,}"
    )
    print(
        f"[ResNet50-MIMCL] mask_ratio="
        f"{args.mask_ratio}, "
        f"mask_patch_size="
        f"{args.mask_patch_size}, "
        f"lambda_cl="
        f"{args.contrast_weight}, "
        f"temperature="
        f"{args.temperature}, "
        f"queue="
        f"{args.queue_size}"
    ...
    ) = build_fixed_reconstruction_preview(
        image_paths=image_paths,
        count=int(
    ...
    logger.info(
        "Fixed MIM reconstruction preview samples: "
    ...
    for epoch in range(
        ...
        for batch_index, (
            ...
            # EMA target is updated from the latest online parameters.
            ...
            if (
                ...
                print(
                    ...
                    f"{momentum:.6f} "
                    f"loss "
                    f"{losses['loss'].item():.4f} "
                    f"mim "
                    f"{losses['loss_mim'].item():.4f} "
                    f"cl "
                    f"{losses['loss_cl'].item():.4f} "
                    f"avg "
                    f"{running_total / count:.4f} "
                    f"grad "
        ...
        print(
            ...
            f"Epoch {epoch}: "
            f"total="
            f"{running_total / len(data_loader):.6f}, "
            f"mim="
            f"{running_mim / len(data_loader):.6f}, "
            f"cl="
            f"{running_cl / len(data_loader):.6f}, "
            f"time="
        ...
        if (
            ...
            save_mimcl_reconstruction_preview(
                model=model,
                preview_batch_cpu=(
                    reconstruction_preview_batch
                ),
        ...
        last_path = (
            output_dir
            / "resnet50_mimcl_last.pth"
        )

        save_mimcl_pretrain_checkpoint(
            path=last_path,
            model=model,
            optimizer=optimizer,
            scaler=scaler,
    ...
    print(
        "[ResNet50-MIMCL] Pretraining complete. "
        f"hours={elapsed / 3600.0:.2f}"
    )
    print(
        "[ResNet50-MIMCL] Online encoder checkpoint: "
        f"{final_path}"
    )
...
def main(config, skip_gradcam=False):
    ...
    logger.info(f"Creating ResNet50-MIMCL fine-tune model: {RESNET50_MIMCL_MODEL_NAME}")

    model = build_resnet50_mimcl(
        num_classes=int(config.MODEL.NUM_CLASSES),
        model_name=RESNET50_MIMCL_MODEL_NAME,
        drop_rate=float(config.MODEL.DROP_RATE),
        drop_path_rate=float(config.MODEL.DROP_PATH_RATE),
        freeze_backbone=FREEZE_BACKBONE,
        pretrained_path=RESNET50_MIMCL_PRETRAINED_PATH,
    )
    model.cuda()
...
def throughput(data_loader, model, logger):
    """
    Measure batched ResNet50-MIMCL inference throughput.
...
if __name__ == "__main__":
    ...
    RESNET50_MIMCL_MODEL_NAME = (
        args.resnet_model
    )
    RESNET50_MIMCL_PRETRAINED_PATH = (
        args.pretrained
    )
    FREEZE_BACKBONE = (
    ...
    if args.stage == "finetune":
        if not RESNET50_MIMCL_PRETRAINED_PATH:
            raise ValueError(
                "--stage finetune requires "
                "--pretrained <resnet50_mimcl_checkpoint.pth>"
            )

    print(
        "ResNet50-MIMCL fine-tune LR "
        "(no SimMIM /512 scaling): "
        f"base_lr="
        f"{config.TRAIN.BASE_LR}, "
        f"warmup_lr="
        f"{config.TRAIN.WARMUP_LR}, "
        f"min_lr="
        f"{config.TRAIN.MIN_LR}"
    )
