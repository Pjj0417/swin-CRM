...
def save_gradcam_images(
    ...
    try:
        ...
        for images, targets in data_loader:
            ...
            for batch_index in range(images.size(0)):
                ...
                # 关闭 aug_smooth，减少 Grad-CAM++ 生成时间。
                # eigen_smooth 可让响应区域更加稳定、连续。
                grayscale_cam = cam(
                    input_tensor=image,
                    targets=cam_targets,
                    aug_smooth=False,
    ...
    logger.info(
        f"Saved {saved_count} detailed Grad-CAM++ images to "
        f"{output_dir}"
    )
