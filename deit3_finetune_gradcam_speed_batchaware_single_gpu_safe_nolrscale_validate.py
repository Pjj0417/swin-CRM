# --------------------------------------------------------
# DeiT-III backbone: local pretrained ImageNet-1K weights
...
# Improved version:
# - keeps DeiT-III + SimMIM compatibility
...
from models.deit3_model import build_deit3
...
DEIT3_MODEL_NAME = "deit3_base_patch16_224.fb_in22k_ft_in1k"
DEIT3_PRETRAINED_PATH = None
...
def parse_option():
    parser = argparse.ArgumentParser(
        "DeiT-III training, evaluation and Grad-CAM++ script",
        add_help=True,
    )
    ...
    parser.add_argument(
        "--deit3-model",
        type=str,
        default="deit3_base_patch16_224.fb_in22k_ft_in1k",
        choices=[
    ...
    parser.add_argument(
        "--freeze-backbone",
        action="store_true",
        help="Freeze DeiT-III backbone and train only the classifier.",
    )
    parser.add_argument(
        "--optimizer",
        type=str,
...
def deit3_reshape_transform(tensor):
    """
    ...
    # Standard DeiT-III has one CLS token followed by patch tokens.
...
def find_deit3_gradcam_target_layers(model_without_ddp, logger=None):
    ...
    # activations suitable for Grad-CAM++ after deit3_reshape_transform().
    target_layer = backbone.blocks[-1].norm1

    if logger is not None:
        logger.info(
            "Selected DeiT-III Grad-CAM++ target layer: "
...
def save_best_weights(
    ...
    checkpoint = {
        ...
        "architecture": DEIT3_MODEL_NAME,
        "initialization": "DeiT3_ImageNet22K_FT_ImageNet1K_pretrained",
    }
...
def save_gradcam_images(
    ...
    try:
        target_layers = find_deit3_gradcam_target_layers(
            model_without_ddp,
            logger,
        )
        ...
        cam = GradCAMPlusPlus(
            ...
            reshape_transform=deit3_reshape_transform,
        )
...
def main(config, skip_gradcam=False):
    ...
    logger.info(f"Creating DeiT-III model: {DEIT3_MODEL_NAME}")

    model = build_deit3(
        num_classes=int(config.MODEL.NUM_CLASSES),
        model_name=DEIT3_MODEL_NAME,
        drop_rate=float(config.MODEL.DROP_RATE),
        drop_path_rate=float(config.MODEL.DROP_PATH_RATE),
        freeze_backbone=FREEZE_BACKBONE,
        pretrained_path=DEIT3_PRETRAINED_PATH,
    )
    model.cuda()
...
if __name__ == "__main__":
    ...
    DEIT3_MODEL_NAME = args.deit3_model
    DEIT3_PRETRAINED_PATH = args.pretrained
    ...
    distributed_backend = "nccl" if world_size > 1 else "gloo"
    print(
        f"Initializing process group: backend={distributed_backend}, "
    ...
    print(
        "DeiT-III fine-tune LR (no SimMIM /512 scaling): "
...
#   deit3_finetune_gradcam_speed_batchaware_single_gpu_safe_nolrscale_validate.py \
#   --cfg ./configs/deit/deit3_base_patch16_224.fb_in22k_ft_in1k_finetune_img224.yaml \
#   --data-path ./data/fatiguev2_105270 \
#   --batch-size 32 \
#   --amp-opt-level O0 \
#   --deit3-model deit3_base_patch16_224.fb_in22k_ft_in1k \
#   --pretrained /root/shared-nvme/uploads/deit3_base_patch16_224.fb_in22k_ft_in1k-b5f2ef4d.pth \
#   --skip-gradcam \
#   --tag deit_base_100ep \
#   --opts \
#   TRAIN.EPOCHS 100 \
