# --------------------------------------------------------
# SimMIM-compatible SwinV2 fine-tuning and Grad-CAM++ script
# SwinV2 backbone: local pretrained ImageNet-1K weights
...
from models.swinv2_model import build_swinv2
...
SWINV2_MODEL_NAME = "swinv2_cr_small_ns_224.sw_in1k"
SWINV2_PRETRAINED_PATH = None
...
def parse_option():
    parser = argparse.ArgumentParser(
        "SwinV2 training, evaluation and Grad-CAM++ script",
        add_help=True,
    )
    ...
    parser.add_argument(
        "--swinv2-model",
        type=str,
        default="swinv2_cr_small_ns_224.sw_in1k",
        choices=[
    ...
    parser.add_argument(
        "--freeze-backbone",
        action="store_true",
        help="Freeze SwinV2 backbone and train only the classifier.",
    )
    parser.add_argument(
        "--optimizer",
        type=str,
...
def swinv2_reshape_transform(tensor):
    """
    Convert SwinV2-CR internal activations to BCHW for Grad-CAM++.
    Supports [B,H,W,C], [B,L,C], and already-BCHW tensors.
    """
    if tensor.ndim == 4:
        # SwinV2-CR block norms operate channels-last.
    ...
    if tensor.ndim == 3:
        ...
        if spatial * spatial != tokens:
            raise RuntimeError(
                f"Cannot reshape {tokens} SwinV2 tokens "
                "into a square feature map."
            )
        return (
            tensor.reshape(batch, spatial, spatial, channels)
            .permute(0, 3, 1, 2)
        )
...
def find_swinv2_gradcam_target_layers(model_without_ddp, logger=None):
    if not hasattr(model_without_ddp, "backbone"):
        raise AttributeError("Model does not contain a SwinV2 backbone.")

    backbone = model_without_ddp.backbone
    if not hasattr(backbone, "stages") or len(backbone.stages) == 0:
        raise AttributeError(
            "SwinV2 backbone does not expose transformer stages."
        )

    final_stage = backbone.stages[-1]
    if not hasattr(final_stage, "blocks") or len(final_stage.blocks) == 0:
        raise AttributeError(
            "Final SwinV2 stage does not expose transformer blocks."
        )

    target_layer = final_stage.blocks[-1].norm2
    if logger is not None:
        logger.info(
            "Selected SwinV2 Grad-CAM++ target layer: "
            "backbone.stages[-1].blocks[-1].norm2"
        )
    return [target_layer]
...
def save_best_weights(
    ...
    checkpoint = {
        ...
        "architecture": SWINV2_MODEL_NAME,
        "initialization": "SwinV2_CR_ImageNet1K_pretrained",
    }
...
def save_gradcam_images(
    ...
    try:
        target_layers = find_swinv2_gradcam_target_layers(
            model_without_ddp,
            logger,
        )
        ...
        cam = GradCAMPlusPlus(
            ...
            reshape_transform=swinv2_reshape_transform,
        )
...
def main(config, skip_gradcam=False):
    ...
    logger.info(f"Creating SwinV2 model: {SWINV2_MODEL_NAME}")

    model = build_swinv2(
        num_classes=int(config.MODEL.NUM_CLASSES),
        model_name=SWINV2_MODEL_NAME,
        drop_rate=float(config.MODEL.DROP_RATE),
        drop_path_rate=float(config.MODEL.DROP_PATH_RATE),
        freeze_backbone=FREEZE_BACKBONE,
        pretrained_path=SWINV2_PRETRAINED_PATH,
    )
    model.cuda()
...
def throughput(data_loader, model, logger):
    """
    Measure batched SwinV2 inference throughput.
...
if __name__ == "__main__":
    ...
    SWINV2_MODEL_NAME = args.swinv2_model
    SWINV2_PRETRAINED_PATH = args.pretrained
    ...
    print(
        "SwinV2 fine-tune LR (no SimMIM /512 scaling): "
...
#   swinv2_finetune_gradcam_speed_batchaware_single_gpu_safe_nolrscale_validate.py  \
#   --cfg ./configs/edgenext/swinv2_cr_small_ns_224.sw_in1k_finetune_img224.yaml \
#   --data-path ./data/fatiguev2_105270 \
#   --batch-size 32 \
#   --amp-opt-level O0 \
#   --swinv2-model swinv2_cr_small_ns_224.sw_in1k \
#   --pretrained /root/shared-nvme/uploads/swinv2_cr_small_ns_224.sw_in1k_usi_in1k.bin \
#   --skip-gradcam \
#   --tag swinv2_cr_small_ns_224.sw_in1k_100ep \
#   --opts \
#   TRAIN.EPOCHS 100 \
