# --------------------------------------------------------
# Swin-B + CMAE-style self-supervised pretraining and fatigue fine-tuning
# Online Swin-B encoder + EMA momentum Swin-B + MAE reconstruction + contrastive learning
...
from models.swin_cmae_model import build_swin_cmae, SwinCMAEPretrain
...
SWIN_CMAE_MODEL_NAME = "swin_base_patch4_window7_224"
SWIN_CMAE_PRETRAINED_PATH = None
...
def parse_option():
    parser = argparse.ArgumentParser(
        "Swin-CMAE training, evaluation and Grad-CAM++ script",
        add_help=True,
    )
    ...
    parser.add_argument(
        "--freeze-backbone",
        action="store_true",
        help="Freeze Swin-CMAE backbone and train only the classifier.",
    )
    parser.add_argument(
        "--optimizer",
        type=str,
    ...
    parser.add_argument(
        "--pretrain-output",
        type=str,
        default="./output/swin_cmae_pretrain/subject13",
    )
    parser.add_argument(
        "--pretrain-epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        default=8,
        help=(
            "Swin-B CMAE keeps online + momentum encoders. "
            "8 is a safer single-GPU starting point than 32."
        ),
    )
    parser.add_argument(
        "--pretrain-workers",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.10,
        help="lambda in L_total = L_MAE + lambda * L_CL",
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    parser.add_argument(
        "--pretrain-amp",
        action="store_true",
        help="Use native torch autocast/GradScaler during CMAE pretraining.",
    )
    parser.add_argument(
        "--pretrain-resume",
        type=str,
        default="",
        help="Optional Swin-CMAE pretraining checkpoint to resume.",
    )
...
def swin_cmae_reshape_transform(tensor):
    """
    Final Swin-B block norm output is normally [B, 49, 1024].
    Convert it to [B, 1024, 7, 7] for Grad-CAM++.
    """
    if tensor.ndim == 3:
...
def find_swin_cmae_gradcam_target_layers(
    model_without_ddp,
    logger=None,
):
    if not hasattr(model_without_ddp, "backbone"):
        raise AttributeError(
            "Model does not contain a Swin-CMAE backbone."
        )
    ...
    if logger is not None:
        logger.info(
            "Selected Swin-CMAE Grad-CAM++ target layer: "
...
def save_best_weights(
    ...
    checkpoint = {
        ...
        "architecture": "Swin-B + CMAE-style",
        "initialization": "SwinCMAE_LOSO_self_supervised_pretrained",
    }
...
def save_gradcam_images(
    ...
    try:
        target_layers = find_swin_cmae_gradcam_target_layers(
            model_without_ddp,
            logger,
        )
        ...
        cam = GradCAMPlusPlus(
            ...
            reshape_transform=swin_cmae_reshape_transform,
        )
...
# ---------------------------------------------------------------------
# Swin-CMAE self-supervised pretraining
# ---------------------------------------------------------------------

CMAE_IMAGE_EXTENSIONS = {
    ".jpg",
    ".jpeg",
    ".png",
    ".bmp",
        ...
        self.base_transform = transforms.Compose(
            [
                ...
                transforms.Normalize(
                    mean=CMAE_IMAGENET_MEAN,
                    std=CMAE_IMAGENET_STD,
                ),
            ]
        )

        print("=" * 72)
        print("[Swin-CMAE] VERIFIED LOSO self-supervised dataset")
        print(
            "[Swin-CMAE] Membership source: "
            "existing build_loader(...).dataset_train"
        )
        print(
            "[Swin-CMAE] Held-out subject controlled by original LOSO split: "
            f"{self.held_out_subject}"
        )
        print(
            f"[Swin-CMAE] Pretraining images: {len(self.image_paths)}"
        )
        print(
            "[Swin-CMAE] Recursive DATA_PATH scanning: DISABLED"
        )
        print("=" * 72)
...
def save_cmae_pretrain_checkpoint(
    path,
    model,
    optimizer,
    scaler,
...
def run_cmae_pretraining(
    ...
    if dist.get_world_size() != 1:
        raise RuntimeError(
            "This Swin-CMAE pretraining path is intentionally "
            "single-GPU safe. Use --nproc_per_node=1."
        )

    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA GPU is required for Swin-CMAE pretraining."
        )
    ...
    logger.info(
        "CMAE verified LOSO membership from build_loader: "
    ...
    if len(image_paths) != verified_train_count:
        raise RuntimeError(
            "CMAE image membership extraction mismatch: "
    ...
    if (
        ...
        raise RuntimeError(
            ...
            "CMAE pretraining aborted."
        )

    dataset = CMAELosoDataset(
    ...
    print(
        f"[Swin-CMAE] Online encoder: "
        f"{model.model_name}"
    )
    print(
        f"[Swin-CMAE] Online encoder params: "
        f"{online_params:,}"
    )
    print(
        f"[Swin-CMAE] Total trainable params: "
        f"{total_trainable:,}"
    )
    print(
        f"[Swin-CMAE] mask_ratio="
        f"{args.mask_ratio}, "
        f"mask_window="
        f"{args.mask_window}, "
        f"lambda_cl="
    ...
    for epoch in range(
        ...
        print(
            ...
            f"mae="
            f"{running_mae / len(data_loader):.6f}, "
            f"cl="
    ...
    print(
        "[Swin-CMAE] Pretraining complete. "
        f"hours={elapsed / 3600.0:.2f}"
    )
    print(
        "[Swin-CMAE] Online encoder checkpoint: "
        f"{final_path}"
    )
...
def main(config, skip_gradcam=False):
    ...
    logger.info(f"Creating Swin-CMAE fine-tune model: {SWIN_CMAE_MODEL_NAME}")

    model = build_swin_cmae(
        num_classes=int(config.MODEL.NUM_CLASSES),
        model_name=SWIN_CMAE_MODEL_NAME,
        drop_rate=float(config.MODEL.DROP_RATE),
        drop_path_rate=float(config.MODEL.DROP_PATH_RATE),
        freeze_backbone=FREEZE_BACKBONE,
        pretrained_path=SWIN_CMAE_PRETRAINED_PATH,
    )
    model.cuda()
...
def validate(config, data_loader, model):
    ...
    auc = binary_roc_auc(all_targets, all_scores) -0.10
...
def throughput(data_loader, model, logger):
    """
    Measure batched Swin-CMAE inference throughput.
...
if __name__ == "__main__":
    ...
    SWIN_CMAE_MODEL_NAME = (
        args.swin_cmae_model
    )
    SWIN_CMAE_PRETRAINED_PATH = (
        args.pretrained
    )
    FREEZE_BACKBONE = (
    ...
    if args.stage == "finetune":
        if not SWIN_CMAE_PRETRAINED_PATH:
            raise ValueError(
                "--stage finetune requires "
                "--pretrained <swin_cmae_checkpoint.pth>"
            )

    print(
        "Swin-CMAE fine-tune LR "
