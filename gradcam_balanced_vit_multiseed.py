...
if __name__ == "__main__":
    ...
    # Reproducible random initialization / data augmentation.
    # Each independent run should pass a different top-level SEED value.
    ...
    # Deterministic cuDNN behavior makes comparisons between seeds meaningful.
    # Some CUDA operations may still be nondeterministic on particular versions.
    cudnn.benchmark = False
    cudnn.deterministic = True

    logger_seed_message = (
        f"Random seed initialized: base_seed={config.SEED}, "
        f"rank={dist.get_rank()}, effective_seed={seed}"
    )
    print(logger_seed_message, flush=True)
    ...
    logger.info(logger_seed_message)
