# --------------------------------------------------------
# ResNet50 ImageNet-1K fine-tuning and Grad-CAM++ adaptation
# --------------------------------------------------------
...
def main(config):
    ...
    _counts = Counter(dataset_train.targets)
    fatigue_count = _counts[dataset_train.class_to_idx["fatigue"]]
    nofatigue_count = _counts[dataset_train.class_to_idx["nofatigue"]]
    ...
    logger.info("=" * 70)
    logger.info("Final maximum validation statistics")
    logger.info("=" * 70)
    logger.info(f"Max Accuracy: {max_accuracy:.3f}%")
    logger.info(f"Max F1-score(fatigue): {max_f1:.3f}%")
    logger.info(f"Max Balanced Accuracy: {max_balanced_accuracy:.3f}%")
    logger.info(f"Max ROC-AUC(fatigue): {max_auc:.4f}")
    logger.info(f"Max Precision(fatigue): {max_precision:.4f}")
    logger.info(f"Max Recall(fatigue): {max_recall:.4f}")
    logger.info(f"Max Specificity(nofatigue): {max_specificity:.4f}")
    logger.info("=" * 70)
