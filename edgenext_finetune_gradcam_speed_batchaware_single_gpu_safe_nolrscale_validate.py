# --------------------------------------------------------
# EdgeNeXt backbone: local pretrained ImageNet-1K weights
...
from models.edgenext_model import build_edgenext
...
EDGENEXT_MODEL_NAME = "edgenext_small"
EDGENEXT_PRETRAINED_PATH = None
...
def parse_option():
    parser = argparse.ArgumentParser(
        "EdgeNeXt training, evaluation and Grad-CAM++ script",
        add_help=True,
    )
    ...
    parser.add_argument(
        "--freeze-backbone",
        action="store_true",
        help="Freeze EdgeNeXt backbone and train only the classifier.",
    )
    parser.add_argument(
        "--optimizer",
        type=str,
...
def find_edgenext_gradcam_target_layers(model_without_ddp, logger=None):
    """
    ...
    if not hasattr(model_without_ddp, "backbone"):
        raise AttributeError("Model does not contain a EdgeNeXt backbone.")
    ...
    if target_layer is None:
        raise RuntimeError(
            "Could not find a Conv2d layer for EdgeNeXt Grad-CAM++."
        )

    if logger is not None:
        logger.info(
            "Selected EdgeNeXt Grad-CAM++ target layer: "
...
def save_best_weights(
    ...
    checkpoint = {
        ...
        "architecture": EDGENEXT_MODEL_NAME,
        "initialization": "EdgeNeXt_ImageNet1K_pretrained",
    }
...
def save_gradcam_images(
    ...
    try:
        target_layers = find_edgenext_gradcam_target_layers(
            model_without_ddp,
            logger,
        )
...
def main(config, skip_gradcam=False):
    ...
    logger.info(f"Creating EdgeNeXt model: {EDGENEXT_MODEL_NAME}")

    model = build_edgenext(
        num_classes=int(config.MODEL.NUM_CLASSES),
        model_name=EDGENEXT_MODEL_NAME,
        drop_rate=float(config.MODEL.DROP_RATE),
        drop_path_rate=float(config.MODEL.DROP_PATH_RATE),
        freeze_backbone=FREEZE_BACKBONE,
        pretrained_path=EDGENEXT_PRETRAINED_PATH,
    )
    model.cuda()
...
def throughput(data_loader, model, logger):
    """
    Measure batched EdgeNeXt inference throughput.
...
if __name__ == "__main__":
    ...
    EDGENEXT_MODEL_NAME = args.edgenext_model
    EDGENEXT_PRETRAINED_PATH = args.pretrained
    ...
    print(
        "EdgeNeXt fine-tune LR (no SimMIM /512 scaling): "
...
#   edgenext_finetune_gradcam_speed_batchaware_single_gpu_safe_nolrscale_validate.py \
#   --cfg ./configs/edgenext/edgenext_small_finetune_img224.yaml \
#   --data-path ./data/fatiguev2_105270 \
#   --batch-size 32 \
#   --amp-opt-level O0 \
#   --edgenext-model edgenext_small \
#   --pretrained /root/shared-nvme/uploads/edgenext_small_usi_in1k.bin \
#   --skip-gradcam \
#   --tag edgenext_small_100ep \
#   --opts \
#   TRAIN.EPOCHS 100 \
