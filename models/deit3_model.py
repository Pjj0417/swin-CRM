...
def _extract_state_dict(checkpoint):
    if not isinstance(checkpoint, dict):
        raise TypeError("Unexpected DeiT-III checkpoint format.")
    ...
    if pretrained_path:
        ...
        if not os.path.isfile(path):
            raise FileNotFoundError(
                "\nDeiT-III pretrained weight not found:\n" + path + "\n"
            )
        return path
    candidates = [
        os.path.join(WEIGHT_DIR, "deit3_base_patch16_224_fb_in22k_ft_in1k.safetensors"),
        os.path.join(WEIGHT_DIR, "deit3_base_patch16_224.safetensors"),
    ]
...
def _create_backbone(model_name, drop_path_rate):
    ...
    try:
        ...
        if "drop_path" not in str(error).lower():
            raise
        print(
            "[DeiT-III] drop_path_rate unsupported; retry without it.",
            flush=True,
        )
...
class DeiT3Classifier(nn.Module):
    def __init__(
        ...
        if model_name not in SUPPORTED_DEIT3_MODELS:
            raise ValueError(
                f"Unsupported DeiT-III model: {model_name}. "
                f"Available: {sorted(SUPPORTED_DEIT3_MODELS)}"
            )

        self.model_name = model_name
        weight_path = _resolve_pretrained_path(pretrained_path)
        print("=" * 70, flush=True)
        print("[DeiT-III] Loading local pretrained model", flush=True)
        print(f"[DeiT-III] Model: {model_name}", flush=True)
        print(f"[DeiT-III] Weight path: {weight_path}", flush=True)
        ...
        print(
            f"[DeiT-III] Pretrained parameter coverage: {coverage * 100:.2f}%",
            flush=True,
        )
        if coverage < 0.95:
            missing_preview = [
            ...
            raise RuntimeError(
                "DeiT-III pretrained checkpoint coverage too low: "
                f"{coverage * 100:.2f}%. "
                f"First unmatched keys: {missing_preview}"
            )

        msg = self.backbone.load_state_dict(state_dict, strict=False)
        print("[DeiT-III] Local pretrained weights loaded", flush=True)
        print(f"[DeiT-III] Missing keys: {len(msg.missing_keys)}", flush=True)
        print(f"[DeiT-III] Unexpected keys: {len(msg.unexpected_keys)}", flush=True)
        print("=" * 70, flush=True)

        if freeze_backbone:
            for p in self.backbone.parameters():
                p.requires_grad = False
            print("[DeiT-III] Backbone frozen", flush=True)
        ...
        print(
            "[DeiT-III] Classification head created: "
