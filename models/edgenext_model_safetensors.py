...
def _extract_state_dict(checkpoint):
    if not isinstance(checkpoint, dict):
        raise TypeError(
            "Unexpected EdgeNeXt checkpoint format. "
...
def _resolve_pretrained_path(pretrained_path=None):
    if pretrained_path:
        ...
        if not os.path.isfile(pretrained_path):
            raise FileNotFoundError(
                "\nEdgeNeXt pretrained weight not found:\n"
                f"{pretrained_path}\n"
            )
    ...
    candidates = [
        ...
        os.path.join(WEIGHT_DIR, 'edgenext_small_usi_in1k.bin'),
        os.path.join(WEIGHT_DIR, 'edgenext_small.pth'),
    ]
...
def _create_backbone(model_name, drop_path_rate):
    ...
    try:
        ...
        print(
            "[EdgeNeXt] drop_path_rate is not supported by this "
...
class EdgeNeXtClassifier(nn.Module):
    """
    EdgeNeXt pretrained backbone + fatigue/nofatigue classifier.
    ...
    def __init__(
        ...
        if model_name != 'edgenext_small':
            raise ValueError(
                f"Unsupported EdgeNeXt model: {model_name}. "
                "Expected edgenext_small."
            )
        ...
        print("=" * 70, flush=True)
        print("[EdgeNeXt] Loading local pretrained model", flush=True)
        print(f"[EdgeNeXt] Model: {model_name}", flush=True)
        print(f"[EdgeNeXt] Weight path: {weight_path}", flush=True)
        print(
            "[EdgeNeXt] Expected normalization: "
        ...
        print("[EdgeNeXt] Local pretrained weights loaded", flush=True)
        print(
            f"[EdgeNeXt] Missing keys: {len(msg.missing_keys)}",
            flush=True,
        )
        print(
            f"[EdgeNeXt] Unexpected keys: {len(msg.unexpected_keys)}",
            flush=True,
        )
        if msg.missing_keys:
            print(
                "[EdgeNeXt] First missing keys:",
                msg.missing_keys[:10],
                flush=True,
            )
        if msg.unexpected_keys:
            print(
                "[EdgeNeXt] First unexpected keys:",
        ...
        if freeze_backbone:
            print("[EdgeNeXt] Backbone frozen", flush=True)
        ...
        print(
            "[EdgeNeXt] Classification head created: "
