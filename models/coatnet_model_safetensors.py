...
def _extract_state_dict(checkpoint):
    if not isinstance(checkpoint, dict):
        raise TypeError(
            "Unexpected CoAtNet checkpoint format. "
...
def _resolve_pretrained_path(pretrained_path=None):
    if pretrained_path:
        ...
        if not os.path.isfile(pretrained_path):
            raise FileNotFoundError(
                "\nCoAtNet pretrained weight not found:\n"
                f"{pretrained_path}\n"
            )
    ...
    candidates = [
        ...
        os.path.join(WEIGHT_DIR, 'coatnet_0_rw_224_sw_in1k.bin'),
        os.path.join(WEIGHT_DIR, 'coatnet_0_rw_224.pth'),
    ]
...
def _create_backbone(model_name, drop_path_rate):
    ...
    try:
        ...
        print(
            "[CoAtNet] drop_path_rate is not supported by this "
...
class CoAtNetClassifier(nn.Module):
    """
    CoAtNet pretrained backbone + fatigue/nofatigue classifier.
    ...
    def __init__(
        ...
        if model_name != 'coatnet_0_rw_224':
            raise ValueError(
                f"Unsupported CoAtNet model: {model_name}. "
                "Expected coatnet_0_rw_224."
            )
        ...
        print("=" * 70, flush=True)
        print("[CoAtNet] Loading local pretrained model", flush=True)
        print(f"[CoAtNet] Model: {model_name}", flush=True)
        print(f"[CoAtNet] Weight path: {weight_path}", flush=True)
        print(
            "[CoAtNet] Expected normalization: "
        ...
        print("[CoAtNet] Local pretrained weights loaded", flush=True)
        print(
            f"[CoAtNet] Missing keys: {len(msg.missing_keys)}",
            flush=True,
        )
        print(
            f"[CoAtNet] Unexpected keys: {len(msg.unexpected_keys)}",
            flush=True,
        )
        if msg.missing_keys:
            print(
                "[CoAtNet] First missing keys:",
                msg.missing_keys[:10],
                flush=True,
            )
        if msg.unexpected_keys:
            print(
                "[CoAtNet] First unexpected keys:",
        ...
        if freeze_backbone:
            print("[CoAtNet] Backbone frozen", flush=True)
        ...
        print(
            "[CoAtNet] Classification head created: "
