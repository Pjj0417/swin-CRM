# --------------------------------------------------------
# Swin-B + MAE + DWMM + CL self-supervised pretraining and fatigue fine-tuning
# Deformable DW-MSA / DSW-MSA + MAE reconstruction + EMA contrastive learning
...
from models.swin_mae_dwmm_cl_model import build_swin_mae_dwmm_cl, SwinMAEDWMMCLPretrain
...
def parse_option():
    parser = argparse.ArgumentParser(
        "Swin-MAE+DWMM+CL training, evaluation and Grad-CAM++ script",
        add_help=True,
    )
    ...
    parser.add_argument(
        ...
        help=(
            "pretrain: MAE+DWMM+CL only; "
            "finetune: fatigue/nofatigue only; "
            "full: MAE+DWMM+CL pretraining then automatic fine-tuning."
        ),
    )
    parser.add_argument(
        "--held-out-subject",
    ...
    parser.add_argument(
        "--pretrain-output",
        type=str,
        default="./output/swin_mae_dwmm_cl_pretrain/subject13",
    )
    parser.add_argument(
        "--pretrain-epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        default=4,
        help=(
            "MAE+DWMM+CL keeps online + EMA DWMM Swin-B encoders. "
            "4 is the safer single-GPU starting point; try 8 if memory allows."
        ),
    )
    parser.add_argument(
        "--pretrain-workers",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.10,
        help="lambda_CL in L_total = L_MAE + lambda_CL * L_CL",
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.996,
        help="Base EMA momentum; cosine-scheduled toward 1.0.",
    )
    parser.add_argument(
        "--queue-size",
        type=int,
    ...
    parser.add_argument(
        ...
        default=4,
        help=(
            "Maximum translation in pixels for the weak positive "
            "EMA contrastive view."
        ),
    )
    parser.add_argument(
        "--pretrain-drop-path",
    ...
    parser.add_argument(
        "--pretrain-amp",
        action="store_true",
        help="Use native torch autocast/GradScaler during MAE+DWMM+CL pretraining.",
    )
    parser.add_argument(
        "--pretrain-resume",
        type=str,
        default="",
        help="Optional MAE+DWMM+CL rolling last checkpoint to resume.",
    )
...
class MAEDWMMLosoDataset(
    ...
    def __init__(
        ...
        if expected_count is not None:
            ...
            if (
                ...
                raise RuntimeError(
                    "MAE+DWMM+CL LOSO membership mismatch: "
        ...
        print("=" * 72)
        print(
            "[Swin-MAE+DWMM+CL] VERIFIED LOSO self-supervised dataset"
        )
        print(
            "[Swin-MAE+DWMM+CL] Membership source: "
            "existing build_loader(...).dataset_train"
        )
        print(
            "[Swin-MAE+DWMM+CL] Held-out subject controlled by original LOSO split: "
            f"{self.held_out_subject}"
        )
        print(
            "[Swin-MAE+DWMM+CL] Pretraining images: "
            f"{len(self.image_paths)}"
        )
        print(
            "[Swin-MAE+DWMM+CL] Recursive DATA_PATH scanning: DISABLED"
        )
        print("=" * 72)
    ...
    def __getitem__(
        ...
        # Same semantic crop, only a weak spatial perturbation for CL.
        target_view = (
            self._pixel_shift_view(
                online_view
            )
        )
...
def mae_dwmm_cl_momentum_value(
    step,
    total_steps,
    base_momentum,
):
    """
    Cosine EMA schedule from base_momentum toward 1.0.
    """
    progress = (
        step
        / max(
...
def save_mae_dwmm_pretrain_checkpoint(
    ...
    checkpoint_data = {
        ...
        "architecture": (
            "Swin-B + MAE + DWMM + CL "
            "(DW-MSA / DSW-MSA + EMA queue contrastive)"
        ),
        "encoder": (
...
def save_mae_dwmm_reconstruction_preview(
    ...
    if logger is not None:
        logger.info(
            "Saved MAE+DWMM+CL reconstruction preview: "
            f"{epoch_path} "
...
def run_mae_dwmm_pretraining(
    ...
    if dist.get_world_size() != 1:
        raise RuntimeError(
            "This Swin-MAE+DWMM+CL pretraining path is intentionally "
            "single-GPU safe. Use --nproc_per_node=1."
        )

    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA GPU is required for Swin-MAE+DWMM+CL pretraining."
        )
    ...
    logger.info(
        "MAE+DWMM+CL verified LOSO membership from build_loader: "
    ...
    if (
        ...
        raise RuntimeError(
            "MAE+DWMM+CL image membership extraction mismatch: "
    ...
    if (
        ...
        raise RuntimeError(
            ...
            "MAE+DWMM+CL pretraining aborted."
        )
    ...
    cl_online_params = sum(
        parameter.numel()
        for parameter
        in list(
            model.feature_decoder.parameters()
        )
        + list(
    ...
    momentum_params = sum(
        ...
        for parameter
        in list(
            model.momentum_backbone.parameters()
        )
        + list(
            model.momentum_projector.parameters()
        )
    )
    total_trainable = sum(
        parameter.numel()
    ...
    print(
        "[Swin-MAE+DWMM+CL] Encoder: "
        f"{model.model_name}"
    )
    print(
        "[Swin-MAE+DWMM+CL] Swin-B configuration: "
        "embed_dim=128, depths=(2,2,18,2), "
        "heads=(4,8,16,32), window=7"
    )
    print(
        "[Swin-MAE+DWMM+CL] Encoder params: "
        f"{encoder_params:,}"
    )
    print(
        "[Swin-MAE+DWMM+CL] Decoder params: "
        f"{decoder_params:,}"
    )
    print(
        "[Swin-MAE+DWMM+CL] Online CL head params: "
        f"{cl_online_params:,}"
    )
    print(
        "[Swin-MAE+DWMM+CL] Frozen EMA branch params: "
        f"{momentum_params:,}"
    )
    print(
        "[Swin-MAE+DWMM+CL] Total trainable params: "
        f"{total_trainable:,}"
    )
    print(
        "[Swin-MAE+DWMM+CL] mask_ratio="
        f"{args.mask_ratio}, "
        "mask_window="
        f"{args.mask_window}, "
        "offset_scale="
    ...
    logger.info(
        "Fixed MAE+DWMM+CL reconstruction preview samples: "
    ...
    for epoch in range(
        ...
        for (
            ...
            # EMA branch tracks the latest completed online update.
        ...
        # Exactly ONE rolling SSL checkpoint.
        # It contains full SSL state so --pretrain-resume remains exact.
        last_path = (
            output_dir
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "Reconstruction preview failed, but the rolling "
                    "MAE+DWMM+CL checkpoint was already saved. "
    ...
    print(
        "[Swin-MAE+DWMM+CL] Pretraining complete. "
        f"hours={elapsed / 3600.0:.2f}"
    )
    print(
        "[Swin-MAE+DWMM+CL] SSL checkpoint: "
        f"{final_path}"
    )
...
def main(config, skip_gradcam=False):
    ...
    logger.info(f"Creating Swin-MAE+DWMM+CL fine-tune model: {SWIN_MAE_DWMM_MODEL_NAME}")

    model = build_swin_mae_dwmm_cl(
...
def train_one_epoch(
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        # IMPORTANT:
        # Do NOT clip after every micro-batch. Clip only when the accumulated
        # gradient is complete, immediately before optimizer.step().
        if should_step:
            ...
            # Keep LR schedule aligned to image/batch progress. With
            # batch=16, accumulation=2 this closely matches the original
            # batch=32 update budget.
            lr_scheduler.step_update(
                epoch * num_steps + idx
            )
        else:
            # Logging only; no clipping and no optimizer update.
            grad_norm = (
                get_grad_norm(
                    grad_parameters
                )
