# --------------------------------------------------------
# CoAtNet backbone: local pretrained ImageNet-1K weights
...
COATNET_MODEL_NAME = "coatnet_0_rw_224"
COATNET_PRETRAINED_PATH = None
...
def parse_option():
    parser = argparse.ArgumentParser(
        "CoAtNet training, evaluation and Grad-CAM++ script",
        add_help=True,
    )
    ...
    parser.add_argument(
        "--freeze-backbone",
        action="store_true",
        help="Freeze CoAtNet backbone and train only the classifier.",
    )
    parser.add_argument(
        "--optimizer",
        type=str,
...
def find_coatnet_gradcam_target_layers(model_without_ddp, logger=None):
    """
    ...
    if not hasattr(model_without_ddp, "backbone"):
        raise AttributeError("Model does not contain a CoAtNet backbone.")
    ...
    if target_layer is None:
        raise RuntimeError(
            "Could not find a Conv2d layer for CoAtNet Grad-CAM++."
        )

    if logger is not None:
        logger.info(
            "Selected CoAtNet Grad-CAM++ target layer: "
...
def save_best_weights(
    ...
    checkpoint = {
        ...
        "architecture": COATNET_MODEL_NAME,
        "initialization": "CoAtNet_ImageNet1K_pretrained",
    }
...
def save_gradcam_images(
    ...
    try:
        target_layers = find_coatnet_gradcam_target_layers(
            model_without_ddp,
            logger,
        )
...
def main(config, skip_gradcam=False):
    ...
    logger.info(f"Creating CoAtNet model: {COATNET_MODEL_NAME}")
    ...
    model = build_coatnet(
        ...
        pretrained_path=COATNET_PRETRAINED_PATH,
    )
    model.cuda()
...
def throughput(data_loader, model, logger):
    """
    Measure batched CoAtNet inference throughput.
...
if __name__ == "__main__":
    ...
    COATNET_MODEL_NAME = args.coatnet_model
    COATNET_PRETRAINED_PATH = args.pretrained
    ...
    print(
        "CoAtNet fine-tune LR (no SimMIM /512 scaling): "
...
#   coatnet_finetune_gradcam_speed_batchaware_single_gpu_safe_nolrscale.py \
#   --cfg ./configs/coatnet/coatnet_0_rw_224_finetune_img224.yaml \
#   --data-path ./data/fatiguev2_105270 \
#   --batch-size 32 \
#   --amp-opt-level O0 \
#   --coatnet-model coatnet_0_rw_224 \
#   --pretrained /root/shared-nvme/uploads/coatnet_0_rw_224_sw_in1k.bin \
#   --skip-gradcam \
#   --tag coatnet0_100ep \
#   --opts \
#   TRAIN.EPOCHS 100 \
