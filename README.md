...
<p align="center">
  <img src="data sample.jog" width="900">
</p>
...
python swin_mae_dwmm_train_gradcam_speed_single_gpu_loso.py \
  ...
  --held-out-subject jiang_pengfei \
  --pretrain-output ./output/swin_mae_dwmm_pretrain/subject13 \
  --output ./output/swin_mae_dwmm_finetune/subject13 \
...
```text
--stage full
    Run self-supervised pretraining followed by downstream fine-tuning.

--cfg
    Path to the Swin-MAE + DWMM YAML configuration file.

--data-path
    Path to the train driver fatigue image dataset.

--held-out-subject
    Subject reserved for LOSO evaluation.

--pretrain-output
    Directory used to save self-supervised pretraining outputs.

--output
    Directory used to save downstream fine-tuning and evaluation outputs.

--tag
    Experiment name used to organize the output files.
```

For a single-GPU run, the same experiment can also be launched with `torchrun`:
...
wraps this command and supplies the remaining training parameters automatically.
